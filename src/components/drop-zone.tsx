    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        while (attempts < maxAttempts) {
          try {
            const { data, error: transcriptionError } =
            ...
            if (!data?.success) {
              throw new Error(
                "Transcription failed: No success response received"
              );
            }

            // Update recording with transcript
            const { data: updatedRecording, error: updateError } =
              await supabase
                .from("recordings")
                .update({
        ...
        throw (
          lastError ||
          new Error("Failed to process recording after multiple attempts")
        );
      } catch (error) {
      ...
      <div className="flex flex-col items-center gap-2">
        {isUploading ? (
          ...
          <>
            ...
            <p className="text-sm text-gray-600">Dra eller drop flac fil her</p>
            <p className="text-xs text-gray-400">Bare flac filer er tillatt</p>
          </>
        )}
      </div>
    </div>
